                ...
                {integrations.into_iter().map(|(name, provider, icon, desc, gradient, enabled)| {
                    ...
                    view! {
                        <div class=format!("rounded-2xl p-6 border border-slate-600/50 hover:border-slate-400/50 transition-all {}", gradient)>
                            ...
                            <p class={format!("text-sm text-gray-200/80 {}", if provider == "google" { "mb-2" } else { "mb-6" })}>{desc}</p>
                            {if provider == "google" {
                                view! {
                                    <p class="text-[11px] text-blue-200/75 italic mb-6">
                                        "Could be easily extended to additional google tools like gmail,..."
                                    </p>
                                }.into_any()
                            } else {
