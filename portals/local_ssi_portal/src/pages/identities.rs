    ...
    view! {
        <div class="space-y-6 text-white">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h2 class="text-2xl font-bold text-white">"Your Profiles"</h2>
                    ...
                    <button 
                        on:click=move |_| {
                            if let Some(win) = web_sys::window() {
                                if !win.confirm_with_message("This will make your active profile discoverable by others. Continue?").unwrap_or(false) {
                                    return;
                                }
                            }
                            ...
                            spawn_local(async move {
                                match api::publish_did(&ab, uid).await {
                                    Ok(_) => {
                                        set_status_message.set(Some(("✅ Profile is now discoverable!".to_string(), true)));
            ...
            <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 text-sm text-blue-300">
                "Your profile is your digital identity. Others will see your name and can connect with you. Click 'Make Discoverable' so contacts can find you."
            </div>
            ...
            {move || {
                ...
                if list.is_empty() {
                    return view! { <div class="p-8 text-center text-slate-500 italic">"No profiles found. Click 'Add New Profile' to create one."</div> }.into_any();
                }
                            ...
                            children=move |item| {
                                ...
                                let on_activate = {
                                    ...
                                    move |_| {
                                        ...
                                        let tt = tt.clone();
                                        spawn_local(async move {
                                            if let Ok(_) = api::activate_identity(&ab, d.clone(), tt).await {
                                                // Optimistic update: immediately reflect the new active DID
                                                // in the UI to avoid race with the async refetch
                                                set_active_did.set(d);
                                                set_refresh_trigger.update(|n| *n += 1);
                                            }
                                        });
                                    }
                                    ...
                                    <div class="p-4 bg-slate-900 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-lg hover:border-slate-600 transition-colors">
                                        ...
                                        <div class="flex justify-between items-start gap-2">
                                            ...
                                            <div class="group relative">
                                                ...
                                                <div class="opacity-0 invisible group-hover:opacity-100 group-hover:visible absolute right-0 top-full mt-2 bg-slate-800 border border-slate-600 rounded-lg shadow-xl p-3 z-50 transition-all">
                                                    <div class="text-[10px] text-slate-400 mb-1">"Unique ID:"</div>
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            <Show when=move || is_inst_for_show>
                                                <span class="px-3 py-1 bg-amber-900/30 text-amber-500 text-[10px] rounded-full border border-amber-500/50 font-bold" title="Visible as a business account">"BUSINESS"</span>
                                            </Show>
                                            <Show 
                                            ...
                                            >
                                                <span class="px-3 py-1 bg-emerald-900/30 text-emerald-500 text-[10px] rounded-full border border-emerald-500/50 font-bold" title="This is your current profile">"ACTIVE"</span>
                                            </Show>
                                            
                                            <Show when=move || published_dids.get().contains(&d_for_show2)>
                                                <span class="px-3 py-1 bg-purple-900/30 text-purple-500 text-[10px] rounded-full border border-purple-500/50 font-bold" title="Others can find you by searching">"DISCOVERABLE"</span>
                                            </Show>
                                        </div>
