...
impl Guest for SsiVault {
    ...
    fn generate_master_seed(user_id: String, derivation_path: String) -> Result<bool, VaultError> {
        ...
        persistence::set(&b_key, &storage_blob).map_err(|e| VaultError::PersistenceError(format!("Persistence error: {:?}", e)))?;
        ...
        persistence::set(&blind_key(&hmac_key), &routing_secret.to_vec()).map_err(|e| VaultError::PersistenceError(format!("Persistence error: {:?}", e)))?;
    ...
    fn create_did_document(user_id: String, gateway_url: String, target_id: String) -> Result<String, VaultError> {
        ...
        let pub_key_hex = ssi_crypto::did::parse_did_twin_pubkey(&did)
            ...
            .ok_or_else(|| VaultError::SigningFailed("Failed to parse public key from DID".to_string()))?;
    ...
    fn ecdh_handshake(did: String, peer_pubkey: Vec<u8>) -> Result<Vec<u8>, VaultError> {
        // 1. Find owner of this DID to get their seed
        let blind_did_user = blind_key(&format!("did_user:{}", did));
        let user_id_bytes = match persistence::get(&blind_did_user) {
            Ok(Some(b)) => b,
            _ => {
        ...
        // 3. Convert our Ed25519 seed to X25519 secret scalar
        let scalar_bytes = ssi_crypto::encryption::ed25519_seed_to_x25519_secret(&seed);
        let static_secret = x25519_dalek::StaticSecret::from(scalar_bytes);

        // 4. Parse peer public key and project Ed25519 -> X25519
        let peer_pub_arr: [u8; 32] = peer_pubkey.try_into()
            .map_err(|_| VaultError::SigningFailed("Invalid peer public key length".to_string()))?;
        
        let peer_x25519_bytes = ssi_crypto::encryption::ed25519_pub_to_x25519(&peer_pub_arr)
            .ok_or_else(|| VaultError::SigningFailed("Failed to project peer public key to X25519".to_string()))?;
        let peer_public = x25519_dalek::PublicKey::from(peer_x25519_bytes);

        // 5. Diffie-Hellman Key Exchange
        let shared_secret = static_secret.diffie_hellman(&peer_public);

        // 6. Enforce strict cryptographic separation via HKDF-SHA256
        let hk = Hkdf::<Sha256>::new(None, shared_secret.as_bytes());
        let mut key_bytes = [0u8; 32];
        hk.expand(b"sovereign:system-handshake", &mut key_bytes)
            .map_err(|_| VaultError::SigningFailed("HKDF failed".to_string()))?;
...
fn blind_set(key: &str, value: &[u8], user_id: &str) -> Result<(), VaultError> {
    let seed_guard = get_master_seed(user_id)?;
    let seed = &seed_guard.0;
    
    let hk = Hkdf::<Sha256>::new(None, seed);
    let mut enc_key_bytes = [0u8; 32];
    hk.expand(b"sovereign:blind-vault:encryption", &mut enc_key_bytes).map_err(|_| VaultError::SigningFailed("HKDF failed".to_string()))?;
    ...
    let ciphertext = cipher.encrypt(nonce, value).map_err(|_| VaultError::SigningFailed("Encryption failed".to_string()))?;

    let mut blob = Vec::with_capacity(24 + ciphertext.len());
    blob.extend_from_slice(&nonce_bytes);
    blob.extend_from_slice(&ciphertext);

    persistence::set(&blind_key(key), &blob).map_err(|e| VaultError::PersistenceError(format!("Persistence error: {:?}", e)))?;
    
    Ok(())
}

fn blind_get(key: &str, user_id: &str) -> Result<Option<Vec<u8>>, VaultError> {
    let b_key = blind_key(key);
    
    let blob = match persistence::get(&b_key) {
        Ok(Some(b)) => b,
        Ok(None) => return Ok(None),
        Err(e) => return Err(VaultError::PersistenceError(format!("Persistence error: {:?}", e))),
    };

    if blob.len() < 24 {
        return Err(VaultError::SigningFailed("Invalid blind blob size".to_string()));
    }
    ...
    let plaintext = cipher.decrypt(nonce, ciphertext).map_err(|_| VaultError::SigningFailed("Decryption failed".to_string()))?;
