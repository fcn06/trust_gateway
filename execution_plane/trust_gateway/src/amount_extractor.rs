...
const CURRENCY_KEYS: &[&str] = &["currency", "currency_code", "iso_currency"];
...
pub fn extract_amount(arguments: &serde_json::Value) -> ExtractedAmount {
    let amount = extract_numeric_amount(arguments).or_else(|| extract_string_amount(arguments));
...
fn extract_currency(args: &serde_json::Value) -> Option<String> {
    ...
    for key in AMOUNT_KEYS {
        if let Some(s) = args.get(*key).and_then(|v| v.as_str()) {
            if s.starts_with('$') || s.contains("USD") {
                return Some("USD".to_string());
            }
            if s.starts_with('€') || s.contains("EUR") {
                return Some("EUR".to_string());
            }
            if s.starts_with('£') || s.contains("GBP") {
