...
pub struct WebauthnSharedState {
    ...
    pub approval_notifier: Arc<dyn trust_core::ports::ApprovalNotifier>,
    pub nats: Option<async_nats::Client>,
    pub kv_stores: Option<HashMap<String, async_nats::jetstream::kv::Store>>,
    pub auth_vault: Arc<crate::auth::AuthVault>,
    ...
    pub transport_ingress: Option<Arc<dyn trust_core::ports::TransportIngress>>,
    pub extension: Option<Arc<dyn std::any::Any + Send + Sync>>,
    ...
    pub messaging_extension: Option<Arc<dyn MessagingExtension>>,
}

#[async_trait::async_trait]
pub trait MessagingExtension: Send + Sync {
...
impl WebauthnSharedState {
    ...
    pub fn new(
        ...
        approval_notifier: Arc<dyn trust_core::ports::ApprovalNotifier>,
        ...
        transport_ingress: Option<Arc<dyn trust_core::ports::TransportIngress>>,
        extension: Option<Arc<dyn std::any::Any + Send + Sync>>,
        messaging_extension: Option<Arc<dyn MessagingExtension>>,
    ) -> Self {
        let gateway_url = config.gateway_url.clone();
        let auth_vault = Arc::new(crate::auth::AuthVault::new(&jwt_key_bytes));
        WebauthnSharedState {
            ...
            oid4vp_rsa_pem: {
                if let Ok(key_path) = std::env::var("OID4VP_RSA_KEY_PATH") {
                    match std::fs::read_to_string(&key_path) {
                        Ok(pem) => pem.trim().to_string(),
                        Err(e) => {
                            tracing::warn!("⚠️ Failed to read OID4VP_RSA_KEY_PATH '{}': {}", key_path, e);
                            String::new()
                        }
                    }
                } else {
                    std::env::var("OID4VP_RSA_PEM")
                        .unwrap_or_default()
