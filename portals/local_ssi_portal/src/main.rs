        ...
        spawn_local(async move {
            if let Ok(resp) = Request::get("/config.json").send().await {
                if let Ok(c) = resp.json::<PortalConfig>().await {
                    if let Some(ref token) = c.cloudflare_beacon_token {
                        if !token.is_empty() {
                            utils::inject_cloudflare_analytics(token);
                        }
                    }
                    set_config.set(c);
                }
        ...
        if logged_in && !api_base.is_empty() {
            spawn_local(async move {
                ...
                if let Ok(list) = api::get_escalation_requests(&api_base, tok.clone()).await {
                    ...
                    set_escalations_count.set(pending_count as i32);
                }
            });
        }
    });
                ...
                <div class="flex h-screen overflow-hidden bg-slate-900">
                    ...
                    <aside class=move || format!("{} bg-slate-800 border-r border-slate-700 md:flex flex-col hidden z-40 transition-all duration-300 relative", if sidebar_collapsed.get() { "w-20" } else { "w-64" })>
                        ...
                        <nav class="flex-1 overflow-y-auto p-4 space-y-6">
                            // Main Actions (no section header to flatten hierarchy)
                            <div class="space-y-1">
                                <button title="Dashboard" on:click=move |_| set_active_section.set("dashboard".to_string()) class=move || format!("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-3 {}", if active_section.get() == "dashboard" { "bg-blue-600/20 text-blue-400" } else { "text-slate-400 hover:bg-slate-700 hover:text-white" })>
                                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                                    <span class=move || if sidebar_collapsed.get() { "hidden" } else { "block" }>"Dashboard"</span>
                                </button>
                                ...
                                <button title="Approvals" on:click=move |_| set_active_section.set("validation".to_string()) class=move || format!("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-3 {}", if active_section.get() == "validation" { "bg-blue-600/20 text-blue-400" } else { "text-slate-400 hover:bg-slate-700 hover:text-white" })>
                                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    <span class=move || if sidebar_collapsed.get() { "hidden" } else { "block" }>"Approvals"</span>
                                    // Dynamic badge for pending approvals
                                    <Show when=move || { escalations_count.get() > 0 }>
                                        <span class=move || format!("bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full ml-auto {}", if sidebar_collapsed.get() { "hidden" } else { "block" })>
                                            {move || escalations_count.get().to_string()}
                                        </span>
                                    </Show>
                                </button>
                            </div>
                            ...
                            // Governance
                            <div>
                                <h3 class=move || format!("text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 transition-opacity {}", if sidebar_collapsed.get() { "opacity-0 invisible h-0" } else { "opacity-100 visible" })>"Governance"</h3>
                                <div class="space-y-1">
                            ...
                            // Account & Settings
                            <div>
                                <h3 class=move || format!("text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 transition-opacity {}", if sidebar_collapsed.get() { "opacity-0 invisible h-0" } else { "opacity-100 visible" })>"Account & Settings"</h3>
                                <div class="space-y-1">
                                    <Show when=move || messaging_enabled>
                                        <button title="People" on:click=move |_| set_active_section.set("contact_requests".to_string()) class=move || format!("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-3 {}", if active_section.get() == "contact_requests" { "bg-purple-600/20 text-purple-400" } else { "text-slate-400 hover:bg-slate-700 hover:text-white" })>
                                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                                            <span class=move || if sidebar_collapsed.get() { "hidden" } else { "block" }>"People"</span>
                                        </button>
                                    </Show>
                                    ...
                                    <Show when=is_agent_allowed>
                                        <button title="B2B Policies" on:click=move |_| set_active_section.set("b2b_policies".to_string()) class=move || format!("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-3 {}", if active_section.get() == "b2b_policies" { "bg-blue-600/20 text-blue-400" } else { "text-slate-400 hover:bg-slate-700 hover:text-white" })>
                                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                            <span class=move || if sidebar_collapsed.get() { "hidden" } else { "block" }>"B2B Policies"</span>
                                        </button>
                                    </Show>
                                </div>
                            </div>
                    ...
                    <Show when=move || show_mobile_menu.get()>
                        <div class="md:hidden fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col p-4 space-y-4 touch-none overflow-y-auto">
                            <div class="flex justify-between items-center mb-4">
                                <button on:click=move |_| { set_active_section.set("dashboard".to_string()); set_show_mobile_menu.set(false); }>
                            ...
                            <div class="ml-2 mt-2 space-y-2">
                                <button on:click=move |_| { set_active_section.set("dashboard".to_string()); set_show_mobile_menu.set(false); } class="block w-full text-left text-blue-400">"Dashboard"</button>
                                <Show when=move || messaging_enabled>
                                ...
                                <div class="flex items-center justify-between">
                                    <button on:click=move |_| { set_active_section.set("validation".to_string()); set_show_mobile_menu.set(false); } class="block text-left text-blue-400">"Approvals"</button>
                                    <Show when=move || { escalations_count.get() > 0 }>
                            ...
                            <div class="mt-4">
                                <h3 class="text-xs font-bold text-slate-500 uppercase">"Governance"</h3>
                                <div class="ml-2 mt-2 space-y-2">
                            ...
                            <div class="mt-4">
                                <h3 class="text-xs font-bold text-slate-500 uppercase">"Account & Settings"</h3>
                                <div class="ml-2 mt-2 space-y-2">
                                    <Show when=move || messaging_enabled>
                                        <button on:click=move |_| { set_active_section.set("contact_requests".to_string()); set_show_mobile_menu.set(false); } class="block w-full text-left text-purple-400">"People"</button>
                                    </Show>
                                    ...
                                    <Show when=is_agent_allowed>
                                        <button on:click=move |_| { set_active_section.set("b2b_policies".to_string()); set_show_mobile_menu.set(false); } class="block w-full text-left text-blue-400">"B2B Policies"</button>
                                    </Show>
                                </div>
                            </div>
                            ...
                            <div class="mt-auto pt-6 border-t border-slate-700">
                                ...
                                <button on:click=move |_| {
                                    ...
                                    delete_cookie("ssi_user_id");
                                    delete_cookie("ssi_registration_cookie");
                                } class="w-full bg-red-600/20 text-red-400 py-3 rounded-lg font-bold flex items-center justify-center gap-2">
                    ...
                    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
                        <div class="bg-amber-600/10 border-b border-amber-600/20 py-1 text-[10px] text-amber-300/80 text-center font-medium flex items-center justify-center gap-2 select-none z-30">
                            <span>"⚠️ Sandbox Preview — Ephemeral test environment."</span>
                        </div>
                        <header class="md:hidden bg-slate-800 border-b border-slate-700 p-4 flex justify-between items-center z-40">
                            <div class="flex items-center gap-4">
                                ...
                                <button on:click=move |_| set_active_section.set("dashboard".to_string())>
                                    <span class="text-xl font-bold bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent cursor-pointer">"Agent in a Box"</span>
                                </button>
                            </div>
                        </header>
                        ...
                        {move || {
                            ...
                            match active_section.get().as_str() {
                                "dashboard" => view! { <pages::Dashboard username=username.get() token=token.get() base_url=api_base.clone() set_active_section=set_active_section /> }.into_any(),
                                "unified_inbox" | "inbox" | "hub" => view! { <pages::Inbox base_url=api_base.clone() token=token.get() policies=policies identities=identities refresh_trigger=refresh_trigger set_refresh_trigger=set_refresh_trigger set_active_section=set_active_section /> }.into_any(),
                                ...
                                "contact_requests" => view! { <pages::ContactsAndInvitations base_url=api_base.clone() token=token.get() identities=identities refresh_trigger=refresh_trigger set_refresh_trigger=set_refresh_trigger policies=policies active_did=active_did set_active_did=set_active_did set_active_section=set_active_section /> }.into_any(),
                                ...
                                "b2b_policies" => view! { <pages::B2bPoliciesManager base_url=api_base.clone() token=token.get() /> }.into_any(),
                                ...
                                _ => view! { <pages::Inbox base_url=api_base.clone() token=token.get() policies=policies identities=identities refresh_trigger=refresh_trigger set_refresh_trigger=set_refresh_trigger set_active_section=set_active_section /> }.into_any(),
                            }
                        }}
                    </main>
