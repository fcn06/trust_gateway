...
pub fn EscalationRequestsSection(
    ...
    let b_url = StoredValue::new(base_url.clone());
    let tok = StoredValue::new(token.clone());

    // WS6.2: Resource for initial load and refetches
    let requests_resource = LocalResource::new(move || {
        let base = b_url.get_value();
        let t = tok.get_value();
        async move {
            set_error_msg.set(String::new());
            match api::get_escalation_requests(&base, t).await {
                Ok(list) => Ok(list),
                Err(e) => {
                    set_error_msg.set(format!("Failed to load: {}", e));
                    Err(e)
                }
            }
        }
    ...
    // Sync resource to items signal
    Effect::new(move |_| {
        if let Some(Ok(list)) = requests_resource.get() {
        ...
        <div class="space-y-6 max-w-[1400px] mx-auto">
            ...
            <div class="space-y-6">
                    <div class="flex items-center justify-between bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                        <div class="space-y-1 max-w-4xl text-sm">
                            <p class="text-gray-300 font-medium">
                                "Review and authorize actions requested by your AI agent."
                            </p>
                            <p class="text-gray-400">
                                "The risk level shows how sensitive the action is. 'CLICK' means one-tap approval; 'RE-AUTH' requires security re-authentication."
                            </p>
                        </div>
                        <button
                            on:click=move |_| requests_resource.refetch()
                    ...
                    <Show when=move || requests_resource.get().is_none()>
                        <div class="text-center py-12">
                            <div class="animate-spin h-10 w-10 border-4 border-blue-500 border-t-transparent rounded-full mx-auto"></div>
                            <p class="text-gray-400 mt-4 font-medium animate-pulse">"Loading approvals..."</p>
                        </div>
                    </Show>
                    ...
                    <Show when=move || requests_resource.get().is_some() && requests.get().is_empty() && error_msg.get().is_empty()>
                        <div class="bg-slate-800/30 rounded-2xl p-12 text-center border text-slate-500 border-dashed border-slate-700">
                            ...
                            <p class="text-slate-300 text-lg font-medium">"No pending approvals."</p>
                    ...
                    <Show when=move || !requests.get().is_empty()>
                        <div class="space-y-4">
                            {move || requests.get().into_iter().map(|req| {
                                ...
                                let base_a = b_url.get_value();
                                let tok_a = tok.get_value();
                                let base_d = b_url.get_value();
                                let tok_d = tok.get_value();
