...
pub trait ProofVerifier: Send + Sync {
    ...
    async fn verify_presentation(&self, callback: ProofCallback)
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn register(
        &self,
        req: crate::agent::RegisterAgentRequest,
    ) -> Result<crate::agent::AgentRecord, StoreError>;

    /// Look up an agent by its ID.
    async fn get(&self, agent_id: &str) -> Result<Option<crate::agent::AgentRecord>, StoreError>;

    /// Update an existing agent's fields.
    async fn update(
        &self,
        agent_id: &str,
        req: crate::agent::UpdateAgentRequest,
    ) -> Result<crate::agent::AgentRecord, StoreError>;

    /// List all registered agents, optionally filtered by status.
    async fn list(
        &self,
        status_filter: Option<crate::agent::AgentStatus>,
...
pub trait Authenticator: Send + Sync {
    ...
    async fn validate_session(&self, token: &str) -> Result<AuthenticatedIdentity, AuthError>;
    ...
    async fn create_reauth_challenge(&self, user_id: &str) -> Result<serde_json::Value, AuthError>;
