...
pub fn map_wit_to_dto(msg: &MlsMessage, envelope: Option<String>) -> PlainDidcommDto {
    ...
    let mut extracted_channel_type = None;
    ...
    if let Some(obj) = body_json.as_object() {
        ...
        if let Some(ct_val) = obj.get("channel_type").and_then(|v| v.as_str()) {
            extracted_channel_type = Some(ct_val.to_string());
        }
        if let Some(prov_val) = obj.get("provenance") {
            if let Ok(prov) = serde_json::from_value::<MessageProvenance>(prov_val.clone()) {
                extracted_provenance = Some(prov);
            }
        }
    }
    ...
    if let Some(env) = &envelope {
        if let Ok(orig_dto) = serde_json::from_str::<serde_json::Value>(env) {
            ...
            if extracted_channel_type.is_none() {
                if let Some(ct_val) = orig_dto.get("channel_type").and_then(|v| v.as_str()) {
                    extracted_channel_type = Some(ct_val.to_string());
                }
            }
            if extracted_provenance.is_none() {
                if let Some(prov_val) = orig_dto.get("provenance") {
                    if let Ok(prov) = serde_json::from_value::<MessageProvenance>(prov_val.clone()) {
                        extracted_provenance = Some(prov);
                    }
                }
    ...
    PlainDidcommDto {
        ...
        envelope,
        alias: None,
        channel_type: extracted_channel_type,
...
pub struct RegistrationCookie {
    ...
    /// Number of lookup attempts (for rate limiting)
    #[serde(default)]
    pub attempts: u32,
}
