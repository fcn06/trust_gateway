...
pub fn subscribe_oid4vp_submit_response(shared: Arc<WebauthnSharedState>) {
    ...
    tokio::spawn(async move {
        ...
        while let Some(msg) = sub.next().await {
            ...
            // ── Update Escalation Request and Notify Gateway ──
            if let Some(kv) = shared.kv_stores.as_ref().and_then(|m| m.get("escalation_requests")) {
                let mut escalation: Option<crate::dto::EscalationRequest> = None;
                let mut found_key = None;

                // 1. Direct lookup
                if let Ok(Some(entry)) = kv.get(&request_id).await {
                    if let Ok(esc) = serde_json::from_slice::<crate::dto::EscalationRequest>(&entry) {
                        escalation = Some(esc);
                        found_key = Some(request_id.clone());
                    }
                }

                // 2. Scan all keys for prefixed key
                if escalation.is_none() {
                    if let Ok(mut keys) = kv.keys().await {
                        while let Some(Ok(key)) = keys.next().await {
                            if key.ends_with(&format!("_{}", request_id)) {
                ...
                if let (Some(mut esc), Some(key)) = (escalation, found_key) {
                    esc.status = "APPROVED".to_string();
                    if let Ok(bytes) = serde_json::to_vec(&esc) {
                        let _ = kv.put(key, bytes.into()).await;
                    }
                    tracing::info!("✅ Escalation request {} updated to APPROVED in KV store", request_id);
                } else {
                    tracing::warn!("⚠️ Escalation request {} not found in KV store to mark APPROVED", request_id);
                }
            }

            // Bridge approval to Trust Gateway via NATS gateway.v1.approval.decision
            let decision_payload = serde_json::json!({
                "approval_id": request_id,
                "decision": "approve",
                "resolved_by": "wallet_oid4vp",
            });
            if let Err(e) = nats.publish(
                ...
                tracing::warn!("⚠️ Failed to publish OID4VP approval decision to gateway: {}", e);
            } else {
                tracing::info!("📩 Bridged OID4VP approval to gateway via NATS (approval_id: {})", request_id);
            }
