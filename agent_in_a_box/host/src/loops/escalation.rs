...
/// NATS listener for escalation requests from the Trust Gateway.
///
        ...
        match nats.subscribe(subject.to_string()).await {
            Ok(mut sub) => {
                while let Some(msg) = sub.next().await {
                    ...
                    let session_jti = parsed.get("session_jti").and_then(|v| v.as_str()).map(|s| s.to_string());
                    ...
                    let conv_ctx = {
                        ...
                        if let Some(jti) = &session_jti {
                            ctx = map.get(jti).cloned();
                        }
                        
                        if ctx.is_none() {
                            ctx = map.get(&requester_did).cloned()
                               .or_else(|| map.get(&user_did).cloned());
                        }
                        
                        if ctx.is_none() {
                            tracing::warn!("⚠️ No conversation context found for session_jti={:?}, requester_did={} or user_did={}. Map keys: {:?}", session_jti, requester_did, user_did, map.keys().collect::<Vec<_>>());
                        } else {
                            tracing::info!("✅ Found conversation context for escalation: thid={:?}", ctx.as_ref().and_then(|c| Some(&c.thid)));
                        }
                        ctx
                    };
                    ...
                    if let Some(kv_stores) = &shared.kv_stores {
                        ...
                        // Store in pending_oid4vp_requests if OID4VP proof is required
                        if proof_required {
                            if let Some(pending_store) = kv_stores.get("pending_oid4vp_requests") {
                                let action_req_for_oid4vp = ssi_crypto::ucan::ActionRequest {
                                    request_id: correlation_id.clone(),
                                    tool_name: tool_name.clone(),
                                    human_summary: reason.clone().unwrap_or_else(|| format!("Verify identity for {}", tool_name)),
                                    payload_hash: String::new(),
                                    expires_at: (chrono::Utc::now().timestamp() + 120) as u64,
                                };
                                match serde_json::to_vec(&action_req_for_oid4vp) {
                                    Ok(bytes) => {
                                        match pending_store.put(correlation_id.clone(), bytes.into()).await {
                                            Ok(_) => tracing::info!("💾 Stored pending OID4VP request for correlation_id={}", correlation_id),
                                            Err(e) => tracing::error!("❌ Failed to store pending OID4VP request in KV: {}", e),
                                        }
                                    }
                                    Err(e) => tracing::error!("❌ Failed to serialize pending OID4VP request: {}", e),
                                }
                            }
                        }
                    }
