...
pub fn load_config() -> Result<HostConfig> {
    ...
    let mut config: HostConfig = loader.try_deserialize()?;
    if let Ok(val) = std::env::var("ALLOWED_AGENT_TENANTS") {
        tracing::info!("📌 Overriding allowed_agent_tenants from env: {}", val);
        config.allowed_agent_tenants = val;
    }
...
pub async fn setup_nats(config: &HostConfig) -> Result<(async_nats::Client, HashMap<String, Store>)> {
    ...
    let mut buckets = vec![
        ...
        "user_profiles", "remote_link_codes", "sovereign_kv", "dht_discovery",
        ...
        "telegram_to_uid", "telegram_rate_limit", "mls_store"
    ];

    #[cfg(feature = "messaging")]
    buckets.push("B2B_POLICIES");
...
pub fn load_server_keys() -> Result<ServerKeys> {
    let path_str = std::env::var("SERVER_KEYS_PATH")
        .unwrap_or_else(|_| "config/server.keys".to_string());
    let path = std::path::Path::new(&path_str);
    let mut keys = if path.exists() {
        let content = std::fs::read_to_string(path)?;
        serde_json::from_str::<ServerKeys>(&content)?
    } else {
        tracing::warn!("⚠️ server.keys not found at {:?}, generating new keys...", path);
