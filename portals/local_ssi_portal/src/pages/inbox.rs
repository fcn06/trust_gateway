...
pub fn Inbox(
    ...
    #[allow(unused_variables)] refresh_trigger: ReadSignal<i32>,
                    ...
                    <div class=move || format!(
                        ...
                        {move || {
                            ...
                            if curr_threads.is_empty() {
                                view! {
                                    <div class="p-8 text-center text-slate-500 mt-10 flex flex-col items-center gap-3">
                                        <p class="italic text-slate-400 font-medium">"No conversations yet."</p>
                                        <p class="text-[11px] text-slate-500 max-w-[200px]">"Add a contact first to start a conversation."</p>
                                    </div>
                                }.into_any()
                            } else {
                                curr_threads.into_iter().map(|(tid, msgs)| {
                                    ...
                                    // Find the contact's DID in this thread
                                    let contact_did_opt = msgs.iter().find_map(|m| {
                                        if let Some(f) = &m.from {
                                            if !my_dids.contains(f) {
                                                return Some(f.clone());
                                            }
                                        }
                                        None
                                        ...
                                        msgs.iter().find_map(|m| {
                                            if let Some(tos) = &m.to {
                                                for t in tos {
                                                    if !my_dids.contains(t) {
                                                        return Some(t.clone());
                                                    }
                                                }
                                            }
                                    ...
                                    let display_name = if is_self_message {
                                        ...
                                        if let Some(policy) = policies.get().iter().find(|p| &p.did == contact_did) {
                                            ...
                                            if contact_did.len() > 16 { format!("{}...{}", &contact_did[..8], &contact_did[contact_did.len()-4..]) } else if !contact_did.is_empty() { contact_did.clone() } else { "Unknown Contact".to_string() }
                                        }
                                    } else {
                                        "Unknown".to_string()
                                    };
                    ...
                    <div class=move || format!(
                        ...
                        <Show when=move || !show_new_msg.get() && selected_thread.get().is_some()>
                            {move || {
                                ...
                                let contact_did_opt = thread_msgs.iter().find_map(|m| {
                                    if let Some(f) = &m.from {
                                        if !my_dids.contains(f) {
                                            return Some(f.clone());
                                        }
                                ...
                                view! {
                                    <div class="flex-1 overflow-y-scroll p-4 space-y-4 flex flex-col">
                                        <div class="flex-1 space-y-2 pr-2">
                                            ...
                                            {thread_msgs.into_iter().map(|msg| {
                                                ...
                                                let bubble_class = if is_outbound { 
                                                    ...
                                                    "bg-slate-700 text-slate-200 rounded-r-xl rounded-tl-xl" 
                                                };
                                                ...
                                                view! {
                                                    <div class=format!("flex {}", align_class)>
                                                        <div class="max-w-[80%] flex flex-col">
                                                            <div class=format!("flex items-baseline gap-1.5 mb-0.5 px-1 {}", if is_outbound { "justify-end" } else { "" })>
                                                                <span class="text-[9px] text-slate-400/80 font-bold uppercase tracking-wider">
                                                                ...
                                                                <span class="text-[8px] text-slate-500 font-mono">{format_timestamp(msg.created_time.unwrap_or(0))}</span>
                                                            </div>
                                                            <div class=format!("py-1.5 px-3 shadow-sm text-sm {} space-y-1 [&>p]:mb-1 [&>p:last-child]:mb-0 [&>h1]:text-base [&>h1]:font-bold [&>ul]:list-disc [&>ul]:ml-3 [&>ol]:list-decimal [&>ol]:ml-3 [&>pre]:bg-slate-900/50 [&>pre]:p-1.5 [&>pre]:rounded-md [&>code]:bg-slate-900/50 [&>code]:px-1 [&>code]:rounded-sm [&>a]:text-blue-300 [&>a]:underline", bubble_class) inner_html=html_output>
                                                            </div>
                                                        </div>
                                                    </div>
                                                }
                                    ...
                                    if threads().is_empty() {
                                        view! {
                                            <div class="space-y-4 flex flex-col items-center">
                                                <p class="text-sm mt-2 max-w-sm text-slate-400">"No conversations yet. Add a contact first, then start a conversation."</p>
                                                <button 
                                                    on:click=move |_| set_active_section.set("contact_requests".to_string())
                                                    class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-semibold transition-all shadow-lg shadow-blue-500/20"
                                                >
                                                    "Go to Contacts"
                                                </button>
                                            </div>
