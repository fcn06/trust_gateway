...
pub async fn delete_contact_request(base_url: &str, id: String, token: String) -> Result<(), String> {
    let resp = Request::delete(&format!("{}/contact_requests/{}", base_url, id))
        .credentials(RequestCredentials::Include)
        .header("Authorization", &auth_header(&token))
        .send()
        .await
...
pub async fn get_b2b_policies(base_url: &str, token: String) -> Result<Vec<B2bPolicy>, String> {
    let resp = Request::get(&format!("{}/b2b/policies", base_url))
        .credentials(RequestCredentials::Include)
        .header("Authorization", &auth_header(&token))
        .send()
        .await
        .map_err(|e| e.to_string())?;
    check_status(&resp)?;
    let response: B2bPoliciesResponse = resp.json().await.map_err(|e| e.to_string())?;
    Ok(response.policies)
}

pub async fn update_b2b_policy(base_url: &str, req: UpdateB2bPolicyRequest, token: String) -> Result<(), String> {
    let resp = Request::post(&format!("{}/b2b/policies", base_url))
        .credentials(RequestCredentials::Include)
        .header("Authorization", &auth_header(&token))
        .header("Content-Type", "application/json")
        .body(serde_json::to_string(&req).unwrap())
