...
use crate::auth::contracts::AuthVerifier;
...
pub async fn finish_registration_logic(shared: Arc<WebauthnSharedState>, session_id: String, response: String) -> Result<(bool, String, Option<RegistrationCookie>)> {
    ...
    // Trigger KEK initialization hook if present
    if let Some(ref ext) = shared.messaging_extension {
        let (hmac_tx, hmac_rx) = oneshot::channel();
        let _ = shared.vault_cmd_tx.send(VaultCommand::GetHmacSecret { 
            user_id: user_id.clone(), 
            resp: hmac_tx 
        }).await;
        if let Ok(Ok(secret)) = hmac_rx.await {
            let _ = ext.on_vault_unlocked(shared.clone(), user_id.clone(), secret).await;
        }
    }

    // 6. Create first identity for user and verify it succeeded
    let (create_tx, create_rx) = oneshot::channel();
    let _ = shared.vault_cmd_tx.send(VaultCommand::CreateIdentity(user_id.clone(), create_tx)).await;
    match create_rx.await {
        Ok(did) if !did.is_empty() => {
            tracing::info!("✅ First identity created for user {}: {}", user_id, did);
        },
        Ok(_) => {
            tracing::error!("❌ CreateIdentity returned empty DID for user {} during registration", user_id);
        },
        Err(e) => {
            tracing::error!("❌ CreateIdentity channel error for user {} during registration: {:?}", user_id, e);
        },
    }
...
pub async fn finish_login_logic(shared: Arc<WebauthnSharedState>, session_id: String, response: String) -> Result<(String, String, String, RegistrationCookie)> {
    ...
    match shared.webauthn.finish_passkey_authentication(&auth_response, &auth_state) {
        Ok(_) => {
            ...
            let token = shared.auth_vault.issue_token(claims);
...
pub async fn extract_claims(shared: &WebauthnSharedState, headers: &HeaderMap) -> Result<MyClaims, StatusCode> {
    ...
    let verified = shared.auth_vault.verify(token)
        .map_err(|_| StatusCode::UNAUTHORIZED)?;
    
    let mut custom = verified.claims().clone();
    custom.jti = verified.jwt_id().map(|s| s.to_string());
    ...
    match unlock_rx.await {
        Ok(Ok(true)) => {
            ...
            if let Some(ref ext) = shared.messaging_extension {
                ...
                if let Ok(Ok(secret)) = hmac_rx.await {
                    let _ = ext.on_vault_unlocked(shared.clone(), user_id.to_string(), secret).await;
                }
            }
            ...
            if let Ok(_) = shared.vault_cmd_tx.send(VaultCommand::GetActiveDid(user_id.to_string(), tx2)).await {
                 match rx2.await {
                    ...
                    _ => {
                        // Active DID is empty but vault is unlocked.
                        // Last-resort: if user has DIDs, set the first non-peer as active.
                        // If user has NO DIDs at all, create one on-the-fly.
                        let candidate = user_dids.iter()
                            .find(|d| !d.starts_with("did:peer"))
                            .or_else(|| user_dids.first())
                            .cloned();
                        
                        let did_to_activate = if let Some(existing) = candidate {
                            tracing::warn!("⚠️ Auto-Heal: No active DID for {} but found existing DID {}. Activating it.", user_id, existing);
                            existing
                        } else {
                            // No DIDs exist at all — create one now
                            tracing::warn!("⚠️ Auto-Heal: No DIDs exist for {}. Creating one on-the-fly.", user_id);
                            let (ct, cr) = oneshot::channel();
                            let _ = shared.vault_cmd_tx.send(VaultCommand::CreateIdentity(user_id.to_string(), ct)).await;
                            match cr.await {
                                Ok(new_did) if !new_did.is_empty() => new_did,
                        ...
                        let _ = shared.vault_cmd_tx.send(VaultCommand::SetActiveDid(
                            user_id.to_string(), did_to_activate.clone(), stx
                        )).await;
                        let _ = srx.await;
                        
                        return Ok(did_to_activate);
                    },
                }
            }
        },
