...
impl CreateIdentityCommand {
    ...
    fn store_seed(&self, did: &str, signing_key: &SigningKey) -> Result<(), String> {
        if get_master_seed(&self.user_id).is_ok() {
            blind_set(&format!("seed:{}", did), &signing_key.to_bytes().to_vec(), &self.user_id).map_err(|e| format!("{:?}", e))?;
        } else {
    ...
    fn update_did_list(&self, did: &str) -> Result<(), String> {
        ...
        if !dids.contains(&did.to_string()) {
            ...
            blind_set(&format!("user_dids:{}", self.user_id), &serde_json::to_vec(&dids).unwrap(), &self.user_id).map_err(|e| format!("{:?}", e))?;
    ...
    /// Only non-peer DIDs are eligible as the default active identity,
    /// since peer DIDs are ephemeral and context-specific.
    fn set_active_if_needed(&self, did: &str) -> Result<(), String> {
        // Skip peer DIDs — they should not silently become the active identity
        if self.is_peer {
            return Ok(());
        }
