...
impl SearchMcpService {
    ...
    async fn vp_search(
        ...
        let duckduckgo_url = reqwest::Url::parse_with_params(
            "https://api.duckduckgo.com/",
            &[("q", search_query.as_str()), ("format", "json")],
        )
        .map_err(|e| {
        ...
        let response = client.get(duckduckgo_url.clone()).send().await.map_err(|e| {
            McpError::invalid_request(
                e.to_string(),
        ...
        let parsed_json_response: serde_json::Value = response.json().await.map_err(|e| {
            McpError::invalid_request(
                e.to_string(),
                Some(json!({"messages": duckduckgo_url.to_string()})),
            )
...
impl ServerHandler for SearchMcpService {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "This server provides a 'vp_search' function to search the internet.",
        )
    }
}
