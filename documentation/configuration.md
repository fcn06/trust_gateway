...
| `TELEGRAM_BOT_TOKEN` | No | Token from @BotFather to enable mobile notifications |
| `TELEGRAM_BOT_ENABLED` | No | Set to `true` to activate the Telegram bot listener |
| `TELEGRAM_WEBHOOK_URL` | No | Public URL for the bot (required for production/webhook mode) |
...
lianxi-community/
├── execution_plane/
│   ├── trust_gateway/            # Policy engine, grant issuance, audit, approval daemon
│   ├── native_skill_executor/    # "The Claw" — OS process skill runner + JTI nonce store
│   ├── connector_mcp_server/     # OAuth2 MCP connector (Google, Stripe, Shopify) — pure executor
│   ├── vp_mcp_server/            # VP-authenticated MCP tools (weather, search) — pure executor
│   └── shared_libs/
...
├── agents/
│   └── ssi_agent/
│       ├── ssi_identity_agent/   # A2A reasoning agent (delegates tool calls to McpAgent)
│       └── ssi_mcp_runtime/      # McpAgent — LLM tool loop, dispatches via NATS to gateway
├── portals/
